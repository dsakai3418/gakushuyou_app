...
if st.session_state.username:
    ...
    # ユーザーの用語データをロード
    # ユーザーシートが存在しない場合、load_data_from_gasは空のDataFrameを返すので、そのまま進行。
    # Masterシートからのコピー処理は不要になったため、この部分のロジックは削除。
        ...
        if filtered_df.empty:
            ...
            for _, row in filtered_df.iterrows():
                ...
                with st.expander(f"**{row['用語 (Term)']}.** （カテゴリ: {row['カテゴリ (Category)']}）", 
                    ...
                    # 展開・非展開の切り替えロジック
                    # アコーディオン内のボタンは、アコーディオンが開閉するたびに再描画されるため、
                    # クリック時に状態を更新し、st.rerun()で即座に反映させる
                    if is_expanded:
