...
if st.session_state.username:
    ...
    def update_progress_for_term(term_id, current_progress_value):
        ...
        idx = idx_list[0] 
        # original_term = df_vocab.loc[idx, '用語 (Term)'] # この行は使用されていないため削除可能
        ...
        if new_progress != current_progress_value:
            ...
            if write_data_to_gas(df_vocab, current_worksheet_name):
                # st.toast(f"'{original_term}' の学習進捗が '{new_progress}' に更新されました！") # トーストメッセージは好みで復活させてもOK
                pass 
            ...
            if current_idx >= total_questions:
                ...
                for i, q in enumerate(questions):
                    ...
                    st.session_state.test_mode['detailed_results'].append({
                        ...
                        'term_definition': q['term_definition'], 
                        'term_example': q['term_example'] 
                    })
