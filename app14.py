...
def load_data_from_gas(sheet_name):
    try:
        ...
        # APIキーを使用しない場合 (GAS側もAPIキーチェックを無効にする必要があります)
        # params = {'sheet': sheet_name, 'action': 'read_data'}
...
def write_data_to_gas(df, sheet_name):
    try:
        ...
        # APIキーを使用しない場合
        # params = {'sheet': sheet_name, 'action': 'write_data'}
...
if st.session_state.username:
    ...
    def start_new_test(test_type, category_filter_for_test):
        ...
        st.session_state.test_mode['current_question'] = None # 明示的にリセット
        ...
        generate_next_question() # 最初の問題生成
    ...
    def generate_next_question():
        ...
        st.session_state.test_mode['answered'] = False # 未回答状態にリセット
        st.session_state.current_answer_selection = None

    # --- 学習進捗を自動更新するヘルパー関数 (学習モードのページ遷移時用) ---
    def update_progress_for_navigation(term_id, current_progress_value):
        ...
        # Not Started または Learning の場合のみ進捗を進める
    ...
    if page == "用語一覧":
        ...
        # 初期ロード時のセッションステートのセットアップ
        # filtered_df_indices が空の場合、または df_vocab が変更された場合に初期化
        if 'learning_mode' not in st.session_state or not st.session_state.learning_mode['filtered_df_indices']:
            st.session_state.learning_mode = {
                'filtered_df_indices': df_vocab.index.tolist(), # 全ての用語のインデックスを初期値として設定
                'current_index_in_filtered': 0,
                'selected_category': '全てのカテゴリ',
                'progress_filter': '全ての進捗'
            }
        ...
        # フィルタリング条件が変わったか、または filtered_df_indices が現在の filtered_df と一致しない場合
        # または、 filtered_df が空になった場合にインデックスをリセット
        if (selected_category_filter != st.session_state.learning_mode['selected_category'] or
            ...
            list(st.session_state.learning_mode['filtered_df_indices']) != list(filtered_df.index.tolist())):
            ...
            st.rerun() # フィルタリング結果が変更されたら即座に再描画

        # ここでエラーチェックを追加
        if filtered_df.empty:
            st.info("この条件に一致する用語は見つかりませんでした。")
            # セッションステートのリセット
            st.session_state.learning_mode['filtered_df_indices'] = [] # 明示的に空に
            st.session_state.learning_mode['current_index_in_filtered'] = 0
            st.stop() # ここで処理を停止
        ...
        with col_prev:
            if st.button("前の用語へ", disabled=(current_display_index_in_filtered == 0)):
                update_progress_for_navigation(current_term_data['ID'], current_term_data['学習進捗 (Progress)']) # 進捗を更新
        ...
        selected_category_for_test = st.selectbox("テストするカテゴリを選択:", all_categories_for_test,
        ...
        if st.session_state.test_mode['type'] is None:
            ...
            with col1:
                if st.button("用語 → 説明テスト", key="start_term_def_test"):
                    start_new_test('term_to_def', selected_category_for_test)
            with col2:
                if st.button("例文 → 用語テスト", key="start_example_term_test"):
                    start_new_test('example_to_term', selected_category_for_test)
            st.info("※例文のない用語は出題されません（例文 → 用語テストの場合）。")
        else:
            # テスト問題の生成に失敗した場合のハンドリング
            if st.session_state.test_mode['current_question'] is None:
                st.warning("このカテゴリではテスト問題が作成できませんでした。用語数や例文の有無を確認してください。")
                if st.button("テストを終了する", key="end_test_no_q"):
                    st.session_state.test_mode['type'] = None
                    st.rerun()
                st.stop()

            st.subheader(f"現在のスコア: {st.session_state.test_mode['score']} / {st.session_state.test_mode['total_questions']}")
            st.write(f"**問題:** {st.session_state.test_mode['current_question']['question_text']}")

            # 回答の選択肢を表示
            with st.form(key=f"question_form_{st.session_state.test_mode['total_questions']}"):
                selected_choice = st.radio("選択肢:", st.session_state.test_mode['choices'], 
                                           key=f"radio_{st.session_state.test_mode['total_questions']}",
                                           disabled=st.session_state.test_mode['answered'])
                
                # 回答を送信ボタン
                submit_answer_button = st.form_submit_button("回答を送信", disabled=st.session_state.test_mode['answered'])
                ...
                if submit_answer_button:
                    ...
                    if selected_choice == correct_answer:
                        ...
                        st.success("正解！🎉")
                        # 正解した用語の進捗を更新
                        term_id_to_update = st.session_state.test_mode['current_question']['question_id']
                        # df_vocabは@st.cache_dataでキャッシュされているため、直接変更しても
                        # 次のst.rerun()まで反映されない可能性があります。
                        # しかし、write_data_to_gasがキャッシュをクリアするので、問題なく動作します。
                        idx_list = df_vocab[df_vocab['ID'] == term_id_to_update].index.tolist()
                        if idx_list: # IDが見つかった場合のみ処理
                            idx_to_update = idx_list[0]
                            current_progress = df_vocab.loc[idx_to_update, '学習進捗 (Progress)']

                            if current_progress == 'Learning':
                                df_vocab.loc[idx_to_update, '学習進捗 (Progress)'] = 'Mastered' # LearningからMasteredへ
                                st.toast(f"'{df_vocab.loc[idx_to_update, '用語 (Term)']}' を習得済みにしました！")
                                write_data_to_gas(df_vocab, current_worksheet_name) # 更新を書き込む
                            elif current_progress == 'Not Started':
                                df_vocab.loc[idx_to_update, '学習進捗 (Progress)'] = 'Learning' # Not StartedからLearningへ
                                st.toast(f"'{df_vocab.loc[idx_to_update, '用語 (Term)']}' の学習を開始しました！")
                                write_data_to_gas(df_vocab, current_worksheet_name) # 更新を書き込む
                    else:
                        st.error(f"不正解... 😔 正解は **{correct_answer}** でした。")
                    
                    # 回答を送信したら、st.rerun() して画面を再描画し、次の問題ボタンを表示させる
                    # このst.rerun()で、画面が更新され、次の問題ボタンが有効になる
                    st.rerun()
            
            # 回答後にのみ表示される次の問題ボタン
            if st.session_state.test_mode['answered']:
                if st.button("次の問題へ", key="next_question_button_after_answer"):
                    generate_next_question() # 次の問題を生成
                    st.rerun() # 画面を再描画して新しい問題を表示

            # テスト終了ボタン
            if st.button("テストを終了する", key="end_test_session"):
                st.session_state.test_mode['type'] = None
                st.rerun()

    elif page == "データ管理":
        st.header("データ管理")
        st.info(f"あなたのデータはGoogleスプレッドシートの '{current_worksheet_name}' シートに保存されています。")
        ...
        if not df_vocab.empty:
            ...
            st.info("現在のすべての用語データをCSVファイルとしてダウンロードできます。")
        else:
            st.info("エクスポートできる用語がありません。")

        st.subheader("用語データの一括インポート (CSV)")
        st.warning("既存のデータは上書きされる可能性があります。事前にデータをダウンロードすることを強く推奨します。")
        ...
        if uploaded_file is not None:
            try:
                ...
                if not all(col in imported_df.columns for col in required_cols):
                    ...
                    for col in ['例文 (Example)', '学習進捗 (Progress)']:
                        if col not in imported_df.columns:
                            imported_df[col] = ''
                    imported_df['学習進捗 (Progress)'] = imported_df['学習進捗 (Progress)'].fillna('Not Started') # デフォルト値

                    if st.button("インポートを実行 (既存データ上書きの可能性あり)"):
                        if write_data_to_gas(imported_df, current_worksheet_name):
                            st.success("CSVデータが正常にインポートされました！")
                            st.rerun()
            except Exception as e:
