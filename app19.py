...
# --- 設定項目 ---
GAS_WEBAPP_URL = "https://script.google.com/macros/s/AKfycbzwTbz24jdyXQv1yvwsKMOW4wHbG4Jnh_7ppp_0M8SlbCkxm3-ytuHpGlvdTEUEPXS4PA/exec" 
...
if st.session_state.username:
    ...
    def generate_questions_for_test(test_type, question_source, category_filter='全てのカテゴリ', num_questions=10):
        ...
        for _, question_term_row in selected_terms.iterrows():
            ...
            questions_list.append({
                ...
                'term_definition': question_term_row['説明 (Definition)'], # 説明を追加
                'term_example': question_term_row['例文 (Example)'] # 例文を追加
            })
    ...
    def start_new_test(test_type, question_source, category_filter):
        ...
        st.session_state.test_mode['current_question_index'] = 0 # 新しいテスト開始時は必ず0から
    ...
    # --- 学習進捗を自動更新するヘルパー関数 (削除) ---
    # 学習モードの進捗更新機能は削除するため、この関数はもう使用しません。
    ...
    if page == "用語一覧":
        ...
        with col_prev:
            if st.button("前の用語へ", disabled=(current_display_index_in_filtered == 0)):
                # update_progress_for_term(current_term_data['ID'], current_term_data['学習進捗 (Progress)']) # 削除
        ...
        if not st.session_state.test_mode['is_active']:
            ...
            if st.session_state.test_mode['questions'] and st.session_state.test_mode['current_question_index'] < len(st.session_state.test_mode['questions']):
                ...
                if st.button("テストを再開", key="resume_test_button"):
                    resume_test()
                    st.stop() # resume_test()内でst.rerun()を呼ぶのでここで停止
        ...
        # テストがアクティブな場合 (問題出題画面)
            ...
            if not questions:
                ...
                if st.button("テストを終了する", key="end_test_no_q"):
                    st.session_state.test_mode['is_active'] = False
                    st.session_state.test_mode['questions'] = [] # 問題リストをクリア
            ...
            if current_idx >= total_questions:
                ...
                for i, q in enumerate(questions):
                    ...
                    st.session_state.test_mode['detailed_results'].append({
                        ...
                        'term_definition': q['term_definition'], # 説明を追加
                        'term_example': q['term_example'] # 例文を追加
                    })
                
                st.session_state.test_mode['score'] = final_score # 最終スコアをセッションステートに保存
                ...
                for i, detail in enumerate(st.session_state.test_mode['detailed_results']):
                    ...
                    st.write(f"　- 説明: {detail['term_definition']}")
                    st.write(f"　- 例文: {detail['term_example']}")
                    st.markdown("---")
                ...
                with st.form(key=f"question_form_{current_idx}"):
                    ...
                    if submit_answer:
                        ...
                        if is_correct_current_q:
                            st.success("正解！🎉")
                            # テストモードでは学習進捗の自動更新は行わないため、以下の行を削除またはコメントアウト
                ...
                with st.expander(f"テスト日時: {row['Date']} | カテゴリ: {row['Category']} | 形式: {row['TestType']} | スコア: {row['Score']} / {row['TotalQuestions']}"):
                    ...
                    try:
                        ...
                        for i, detail in enumerate(details):
                            ...
                            st.write(f"　- 正解: {detail['correct_answer']}")
                            st.write(f"　- あなたの回答: {detail['user_answer']}")
                            st.write("---辞書情報---") # 区切り線を変更
                            st.write(f"　- 用語: {detail['term_name']}")
                            st.write(f"　- 説明: {detail['term_definition']}")
                            if pd.notna(detail['term_example']) and detail['term_example'] != '':
                                st.write(f"　- 例文: {detail['term_example']}")
                            st.markdown("---")
                    except json.JSONDecodeError:
