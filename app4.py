...
if st.session_state.username:
    ...
    if 'test_mode' not in st.session_state:
        st.session_state.test_mode = {
            ...
            'total_questions': 0,
            'answered': False,
            'selected_category_filter': '全てのカテゴリ' # ★★★ テストモード用カテゴリフィルタを追加 ★★★
        }
    ...
    # --- テスト問題生成ヘルパー関数 (修正) ---
    def generate_question(test_type, vocab_df, category_filter='全てのカテゴリ'): # ★★★ category_filter引数を追加 ★★★
        # カテゴリで絞り込み
        ...
        if test_type == 'term_to_def':
            ...
            all_options_pool = vocab_df['説明 (Definition)'].tolist() # 絞り込まれたvocab_dfから選択肢を生成
            ...
            if pd.isna(question_term_row['例文 (Example)']) or question_term_row['例文 (Example)'] == '':
                ...
                return generate_question(test_type, vocab_df, category_filter) 
            question_text = question_term_row['例文 (Example)']
            correct_answer = question_term_row['用語 (Term)']
            all_options_pool = vocab_df['用語 (Term)'].tolist() # 絞り込まれたvocab_dfから選択肢を生成
        
        # 不正解の選択肢を選ぶ（最大3つ）
        incorrect_choices = []
        possible_incorrects = [opt for opt in all_options_pool if opt != correct_answer]
        # 選択肢が足りない場合は、絞り込みを緩めるか、選択肢の数を減らすなどの対応も検討
    ...
    # --- テストモードの開始・リセット (修正) ---
    def start_new_test(test_type, category_filter_for_test): # ★★★ category_filter_for_test引数を追加 ★★★
        ...
        st.session_state.test_mode['selected_category_filter'] = category_filter_for_test # ★★★ フィルタをセッションに保存 ★★★
        st.session_state.current_answer_selection = None
        generate_next_question()

    # --- テストモードの次の問題生成 (修正) ---
    def generate_next_question():
        # generate_questionに現在のカテゴリフィルタを渡す
        st.session_state.test_mode['current_question'] = generate_question(
            st.session_state.test_mode['type'], 
            df_vocab, 
            st.session_state.test_mode['selected_category_filter'] # ★★★ ここでフィルタを適用 ★★★
        )
    ...
    page = st.sidebar.radio("Go to", ["用語一覧", "用語の追加・編集", "学習ビュー", "テストモード", "データ管理"])
    ...
    if page == "用語一覧":
        ...
        # テストの開始条件を見直し
        # 1. テストタイプが変わった 2. カテゴリフィルタが変わった 3. 「テストをやり直す」ボタンが押された
        if (st.session_state.test_mode['type'] != selected_test_type or
            st.session_state.test_mode['selected_category_filter'] != selected_category_filter_for_test or # ★★★ フィルタ変更も開始条件に追加 ★★★
            st.sidebar.button("テストをやり直す", key="reset_test_button")):
            
            start_new_test(selected_test_type, selected_category_filter_for_test) # ★★★ フィルタを渡す ★★★
            st.rerun()
